...
Webcam2::Webcam2(std::string device)
{
	...
	int formatCount = Webcam2::listFormats(devName, &formats);
...
int Webcam2::getPacket(AVPacket **outPacket)
{
...
bool Webcam2::hasCodec(const std::string &device, uint32_t fourcc)
{
	bool result = false;
...
bool Webcam2::hasCodec(struct v4l2_fmtdesc *formats, int formatCount, uint32_t fourcc)
{
	bool result = false;
	for(int i = 0; i < formatCount; ++i) {
		if(formats[i].pixelformat == fourcc) {
...
int Webcam2::listFormats(const std::string &device, struct v4l2_fmtdesc **formats)
{
